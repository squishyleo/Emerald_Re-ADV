...
{
    [BATTLE_TERRAIN_GRASS]      = MOVE_SLEEP_POWDER, // STUN_SPORE
    ...
    [BATTLE_TERRAIN_BUILDING]   = MOVE_TRI_ATTACK, // SWIFT
    [BATTLE_TERRAIN_PLAIN]      = MOVE_MAGNITUDE // SWIFT
};
...
static bool8 AccuracyCalcHelper(u16 move)
{
    ...
    if ((WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_RAIN) && gBattleMoves[move].effect == EFFECT_THUNDER)
     || (WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_HAIL) && gBattleMoves[move].effect == EFFECT_BLIZZARD) // ADDED
...
static void Cmd_setrain(void)
{
    ...
    else
    {
        ...
        gWishFutureKnock.weatherDuration = 8; //5
    }
    gBattlescriptCurrInstr++;
}
...
static void Cmd_setreflect(void)
{
    ...
    else
    {
        ...
        gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].reflectTimer = 8; //5
...
static void Cmd_setlightscreen(void)
{
    ...
    else
    {
        ...
        gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].lightscreenTimer = 8; //5
...
static void Cmd_setsafeguard(void)
{
    ...
    else
    {
        ...
        gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].safeguardTimer = 8; //5
